...
class EpisodicMemory(object):
    ...
    def update_memory(self, q_base=0):
        ...
        for traj in trajs:
            ...
            # approximate_qs_1 = self.compute_approximate_return(self.replay_buffer[traj], self.action_buffer[traj])
            # approximate_qs_2 = self.compute_approximate_return(self.replay_buffer[traj], self.action_buffer[traj])
            # approximate_qs_1 = approximate_qs_1.reshape(-1)
            # approximate_qs_2 = approximate_qs_2.reshape(-1)
            # approximate_qs = np.mean([approximate_qs_1,approximate_qs_2],axis=0)
            ...
            for i, s in enumerate(traj):
                approximate_q = self.reward_buffer[s] + self.gamma * (1 - self.truly_done_buffer[s]) * (
    ...
    def update_sequence_with_qs(self, sequence):
        ...
        next_id = -1
        Rtd = 0
        for obs, a, z, encoded_a, q_t, r, truly_done, done in reversed(sequence):
            # print(np.mean(z))
            if truly_done:
                Rtd = r
            else:
